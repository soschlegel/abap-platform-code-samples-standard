...
CLASS cl_sadl_based_segw_project DEFINITION.
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor IMPORTING io_segw_project_name TYPE  /iwbep/i_sbd_ga-project
                                  iv_dpc_only          TYPE boole_d OPTIONAL
                        RAISING   cx_parameter_invalid.

    METHODS get_sadl_xml RETURNING VALUE(sadl_xml) TYPE string
                         RAISING   cx_sadl_gw_no_service_api.

    METHODS get_merged_sadl_xml IMPORTING sadl_xml_source_project   TYPE string
                                          sadl_xml_extended_project TYPE string
                                RETURNING VALUE(sadl_xml_merged)    TYPE string.

    METHODS get_dpc_ext_class_name RETURNING VALUE(class_name)   TYPE string.
    METHODS get_mpc_ext_class_name RETURNING VALUE(class_name)   TYPE string.
    METHODS get_dpc_class_name     RETURNING VALUE(class_name)   TYPE string.
    METHODS get_mpc_class_name     RETURNING VALUE(class_name)   TYPE string.
    METHODS get_segw_project_name  RETURNING VALUE(project_name) TYPE string.
    " methods A012345678901234567890123456789.

    METHODS get_mpc_type_names
      IMPORTING i_cds_view_names                TYPE table_of_strings
      RETURNING VALUE(r_mpc_types_of_cds_views) TYPE tt_mpc_type_of_cds_view.

    METHODS insert_sadl_xml_into_dpc
      IMPORTING iv_sadl_xml                   TYPE string
                iv_src_segw_project_name      TYPE string
      CHANGING  generated_code_definition     TYPE rswsourcet
    ...
    METHODS create_get_entity_set_methods
      IMPORTING cds_view_names                TYPE table_of_strings
      CHANGING  generated_code_definition     TYPE rswsourcet
                generated_code_implementation TYPE rswsourcet.
...
CLASS cl_sadl_based_segw_project IMPLEMENTATION.
  METHOD constructor.
    ...
    DATA service_builder_dm_manager TYPE REF TO /iwbep/if_sbdm_manager.
    DATA lt_range                   TYPE /iwbep/if_sbdm_manager=>ty_t_range_prjct_name.

    segw_project_name = io_segw_project_name.

    mv_dpc_only = iv_dpc_only.
    ...
    LOOP AT segw_project->get_generated_artifacts( ) INTO DATA(generated_artifact).
      ...
      DATA(mpc_ext_suffix) = substring( val = class_name
                                        len = strlen( '_MPC_EXT' )
                                        off = strlen( class_name ) - strlen( '_MPC_EXT' ) ).
      DATA(dpc_ext_suffix) = substring( val = class_name
                                        len = strlen( '_DPC_EXT' )
                                        off = strlen( class_name ) - strlen( '_DPC_EXT' ) ).
      " TODO: variable is assigned but never used (ABAP cleaner)
      DATA(mpc_suffix) = substring( val = class_name
                                    len = strlen( '_MPC' )
                                    off = strlen( class_name ) - strlen( '_MPC' ) ).
      DATA(dpc_suffix) = substring( val = class_name
                                    len = strlen( '_DPC' )
                                    off = strlen( class_name ) - strlen( '_DPC' ) ).
...
*    ENDTRY.
  ...
  METHOD get_cds_views_from_sadl_xml.
    ...
    DATA(ls_sadl_definition) = cl_bsa_sadl_m2m_transformer=>get_instance( )->xml_to_abap_structure(
                                   iv_sadl_xml = sadl_xml ).
    ...
    LOOP AT ls_sadl_definition-structures REFERENCE INTO DATA(lr_structure).
      ...
      IF line_exists( ls_sadl_definition-data_sources[ name = lr_structure->data_source
                                                       type = 'CDS' ] ) AND lr_structure->exposure = 'TRUE'.
  ...
  METHOD create_get_entity_set_methods.
    ...
    LOOP AT cds_view_names INTO DATA(cds_view_name).
      ...
      IF strlen( cds_view_name ) > remaining_num_characters - suffix_length.
        method_name_get_entityset = substring( val = cds_view_name
                                               len = remaining_num_characters - suffix_length ) && get_entityset_suffix.
        method_name_get_entity = substring( val = cds_view_name
                                            len = remaining_num_characters - suffix_length ) && get_entity_suffix.
      ELSE.
        method_name_get_entityset = cds_view_name && get_entityset_suffix.
        method_name_get_entity = cds_view_name && get_entity_suffix.
      ENDIF.

      IF mv_dpc_only = abap_true.
        CONTINUE.
      ENDIF.
  ...
  METHOD create_generic_get_methods.
    ...
    LOOP AT cds_view_names INTO DATA(cds_view_name).
      ...
      IF strlen( cds_view_name ) > remaining_num_characters - suffix_length.
        ...
        method_name_get_entity = substring( val = cds_view_name
                                            len = remaining_num_characters - suffix_length ) && get_entity_suffix.
        tab_name_entityset = substring( val = cds_view_name
                                        len = remaining_num_characters - suffix_length ) && tab_entityset_suffix.
        tab_name_entity = substring( val = cds_view_name
                                     len = remaining_num_characters - suffix_length ) && tab_entity_suffix.
      ELSE.
        method_name_get_entityset = cds_view_name && get_entityset_suffix.
        method_name_get_entity = cds_view_name && get_entity_suffix.
      ...
      ELSE.
        ...
        APPEND |WHEN '{ cds_view_name }'.  | TO generated_code_get_entityset ##NO_TEXT.
        APPEND |    CREATE DATA er_entityset TYPE TABLE OF { mpc_class_name }=>ts_{  cds_view_name }type. | TO generated_code_get_entityset ##NO_TEXT.
        APPEND |    if_sadl_gw_dpc_util~get_dpc( )->get_entityset( EXPORTING io_tech_request_context = io_tech_request_context  | TO generated_code_get_entityset ##NO_TEXT.
        APPEND |                                                   IMPORTING et_data                 = er_entityset->*  | TO generated_code_get_entityset ##NO_TEXT.
        APPEND |                                                             es_response_context     = es_response_context ). | TO generated_code_get_entityset ##NO_TEXT.

        APPEND |WHEN '{ cds_view_name }'.  | TO generated_code_get_entity ##NO_TEXT.
        APPEND |   if_sadl_gw_dpc_util~get_dpc( )->get_entity( EXPORTING io_tech_request_context = io_tech_request_context | TO generated_code_get_entity ##NO_TEXT.
        APPEND |                                               IMPORTING es_data                 = er_entity ). | TO generated_code_get_entity ##NO_TEXT.
    ...
    LOOP AT i_cds_view_names INTO DATA(cds_view_name).
      ...
      LOOP AT et_type INTO DATA(ls_type).
        ...
        LOOP AT result_tab INTO DATA(result).
          ...
          " TODO: variable is assigned but never used (ABAP cleaner)
          DATA(character_before) = substring( val = ls_type-typesrc
                                              off = result-offset - 1
                                              len = 1 ).
          DATA(character_after) = substring( val = ls_type-typesrc
                                             off = result-offset + result-length
                                             len = 1 ).
...
START-OF-SELECTION.
  ...
  PARAMETERS: src_proj TYPE /iwbep/i_sbd_ga-project,
              ...
              dpc_only TYPE boole_d,
              use_dspl TYPE boole_d.
  ...
  TRY.
      ...
      DATA(extended_segw_project) = NEW cl_sadl_based_segw_project( io_segw_project_name = ext_proj
                                                                    iv_dpc_only          = dpc_only ).
...
*      CATCH /iwbep/cx_sbcm_exception.
      ...
      ELSE.
        ...
        cl_gui_frontend_services=>gui_download( EXPORTING  filename = filename
                                                CHANGING   data_tab = gui_download_data
    ...
    CATCH cx_sadl_gw_no_service_api INTO DATA(sadl_gw_no_service_api_exc).
      ...
      IF sadl_gw_no_service_api_exc->cx_sadl_gw_no_service_api-msgno = '004'.
        WRITE / |Exception occurred: { sadl_gw_no_service_api_exc->project_name } is not a RDS based SEGW project|.
      ELSE.
        WRITE / |Exception occurred: { sadl_gw_no_service_api_exc->get_longtext( ) } |.
      ENDIF.
    ...
    CATCH cx_root INTO DATA(sadl_merge_exception).
      ...
      WRITE / |Exception occurred: { root_exception_text }|.
